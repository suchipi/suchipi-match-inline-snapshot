...
test("error message on non-matching snapshot", async () => {
  ...
  expect(run.cleanResult()).toMatchInlineSnapshot(`
    ...
    TypeError: Expected value of type objectWithOnlyTheseProperties({ shouldUpdateOutdated: boolean, shouldCreateNew: boolean, isAllowedToChangeSnapshots: boolean, fsDelegate: objectWithProperties({ readFileAsUtf8: anyFunction, writeUtf8ToFile: anyFunction }), sourceMaps: anyObject, serializers: arrayOf(anyFunction), parserOptions: anyObject, updateScheduling: union(exactString("auto"), exactString("manual")), callStructures: objectWithOnlyTheseProperties({ normal: objectWithOnlyTheseProperties({ astPattern: objectWithProperties({ type: string }), snapshotPath: arrayOf(union(string, number)), stackOffset: number }), forceUpdate: objectWithOnlyTheseProperties({ astPattern: objectWithProperties({ type: string }), snapshotPath: arrayOf(union(string, number)), stackOffset: number }) }), indentation: objectWithOnlyTheseProperties({ tabSize: number, output: union(exactString("tabs"), exactString("spaces")) }) }), but received {"shouldUpdateOutdated":false,"shouldCreateNew":true,"isAllowedToChangeSnapshots":true,"fsDelegate":{"readFileAsUtf8":"<Function readFileAsUtf8>","writeUtf8ToFile":"<Function writeUtf8ToFile>"},"sourceMaps":{},"serializers":["<Function format>","<Function>"],"parserOptions":{},"updateScheduling":"auto","callStructures":{"normal":{"astPattern":{"type":"CallExpression","callee":{"type":"Identifier","name":"matchInlineSnapshot"}},"snapshotPath":["arguments",1],"stackOffset":0},"forceUpdate":{"astPattern":{"type":"CallExpression","callee":{"type":"MemberExpression","object":{"type":"Identifier","name":"matchInlineSnapshot"},"property":{"type":"Identifier","name":"u"}}},"snapshotPath":["arguments",1],"stackOffset":0}},"indentation":{"tabSize":2,"output":"spaces"},"callStructure":{}}
        at somewhere
