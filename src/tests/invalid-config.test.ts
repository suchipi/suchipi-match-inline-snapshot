...
test("error message on non-matching snapshot", async () => {
  ...
  expect(cleanResult(run.result)).toMatchInlineSnapshot(`
    ...
    TypeError: Expected value of type objectWithOnlyTheseProperties({ shouldUpdateOutdated: boolean, shouldCreateNew: boolean, isAllowedToChangeSnapshots: boolean, fsDelegate: objectWithProperties({ readFileAsUtf8: anyFunction, writeUtf8ToFile: anyFunction }), sourceMaps: anyObject, serializers: arrayOf(anyFunction), parserOptions: anyObject, updateScheduling: union(exactString("auto"), exactString("manual")), callStructure: objectWithOnlyTheseProperties({ astPattern: objectWithProperties({ type: string }), snapshotPath: arrayOf(union(string, number)), stackOffset: number }) }), but received {"shouldUpdateOutdated":false,"shouldCreateNew":true,"isAllowedToChangeSnapshots":true,"fsDelegate":{"readFileAsUtf8":"<Function readFileAsUtf8>","writeUtf8ToFile":"<Function writeUtf8ToFile>"},"sourceMaps":{},"serializers":["<Function format>","<Function>"],"parserOptions":{},"updateScheduling":"auto","callStructure":{}}
        at somewhere
